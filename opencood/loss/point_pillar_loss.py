        ...
        if pbar is None:
            print("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f"
                " || Loc Loss: %.4f" % (
                    epoch, batch_id + 1, batch_len,
                    total_loss.item(), conf_loss.item(), reg_loss.item()))
        elif not da_loss['GRL']:
            # pbar.set_description("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f"
            #       " || Loc Loss: %.4f" % (
            #           epoch, batch_id + 1, batch_len,
            #           total_loss.item(), conf_loss.item(), reg_loss.item()))
            # pbar.set_description("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f"
            #       " || Loc Loss: %.4f || D_loss: %.4f || D_loss_s: %.4f || D_loss_t: %.4f ||  D_loss_s_e: %.4f || D_loss_s_e: %.4f || G_loss: %.4f || G_loss: %.4f || " % (
            #           epoch, batch_id + 1, batch_len,
            #           total_loss.item(), conf_loss.item(), reg_loss.item(),
            #           da_loss['D_loss'].item(), da_loss['D_loss_s'].item(), da_loss['D_loss_t'].item(), da_loss['D_loss_s_e'].item(), da_loss['D_loss_t_e'].item(), da_loss['G_loss'].item(), da_loss['final_loss'].item()))
            pbar.set_description("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f"
                  " || Loc Loss: %.4f || D_loss: %.4f || D_loss_t: %.4f ||   D_loss_t_e: %.4f || G_loss: %.4f || final_loss: %.4f || " % (
                      epoch, batch_id + 1, batch_len,
                      total_loss.item(), conf_loss.item(), reg_loss.item(),
                      da_loss['D_loss'].item(), da_loss['D_loss_t'].item(), da_loss['D_loss_t_e'].item(), da_loss['G_loss'].item(), da_loss['final_loss'].item()))
            

            writer.add_scalar('D_loss', da_loss['D_loss'].item(), 
                            epoch*batch_len + batch_id)
            writer.add_scalar('D_loss_s', da_loss['D_loss_s'].item(),
                            epoch*batch_len + batch_id)
            writer.add_scalar('D_loss_t', da_loss['D_loss_t'].item(), 
                            epoch*batch_len + batch_id)
            writer.add_scalar('D_loss_s_e', da_loss['D_loss_s_e'].item(),
                            epoch*batch_len + batch_id)
            writer.add_scalar('D_loss_t_e', da_loss['D_loss_t_e'].item(), 
                            epoch*batch_len + batch_id)
            writer.add_scalar('G_loss', da_loss['G_loss'].item(), 
                            epoch*batch_len + batch_id)
            writer.add_scalar('final_loss', da_loss['final_loss'].item(), 
                            epoch*batch_len + batch_id)
